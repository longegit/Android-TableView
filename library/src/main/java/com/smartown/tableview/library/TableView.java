...
public class TableView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            titleTextSize = typedArray.getDimensionPixelSize(R.styleable.TableView_titleTextSize, Util.dip2px(getContext(), 10));
            titleTextColor = typedArray.getColor(R.styleable.TableView_titleTextColor, Color.parseColor("#999999"));
            tableTitle = typedArray.getString(R.styleable.TableView_tableTitle);
            typedArray.recycle();
        } else {
            unitColumnWidth = 0;
    ...
    private void drawFramework(Canvas canvas) {
        ...
        for (int i = 0; i < columnCount + 1; i++) {
            ...
            if (!TextUtils.isEmpty(tableTitle)) {
                canvas.drawRect(columnLefts[i], rowHeight + dividerWidth, columnLefts[i] + dividerWidth, getHeight(), paint);
            } else {
        ...
        if (!TextUtils.isEmpty(tableTitle)) {
            canvas.drawRect(0, (rowCount + 1) * (rowHeight + dividerWidth), getWidth(), (rowCount + 1) * (rowHeight + dividerWidth) + dividerWidth, paint);
        }
    }
    ...
    private void drawContent(Canvas canvas) {
        float tableBeginY = 0;
        if (!TextUtils.isEmpty(tableTitle)) {
            tableBeginY = rowHeight + dividerWidth;
            paint.setColor(titleTextColor);
            paint.setTextSize(titleTextSize);
            canvas.drawText(tableTitle, getWidth() / 2, getTextBaseLine(0, paint), paint);
        }
            ...
            for (int j = 0; j < columnCount; j++) {
                if (rowContent.length > j) {
                    canvas.drawText(rowContent[j],
                            ...
                            getTextBaseLine(i * (rowHeight + dividerWidth) + tableBeginY, paint),
                            paint);
                }
            }
            if (i == 0) {
     ...
     * @param title
     */
    public TableView setTitle(String title) {
        tableTitle = title;
        return this;
    }
